...
function addDept() {
    inquirer.prompt([
        ...
        db.connection.query(
            ...
            function (err, res) {
                if (err) throw err;
                console.log(`Nice You have added the department ${newDeptResults.addDeptAnswer}`.underline.brightGreen)
                continueOption();
            })
    }
    )
        ...
        inquirer.prompt([
            ...
            if (newRoleResults.addRoleSalary != parseInt(newRoleResults.addRoleSalary)) {
                ...
                db.connection.query("INSERT INTO role SET ?",
                    ...
                    function (err, res) {
                        if (err) throw err;
                        console.log(`Great You have added the role ${newRoleResults.addRoleTitle}`.underline.brightGreen)
                        continueOption();
                    })
            }
        })
...
function addEmployee() {
    db.connection.query("SELECT * FROM role", function (err, res) {
        ...
        inquirer.prompt([
            ...
            {
                type: "confirm",
                message: "And will this person be a manager?",
            ...
            let query = db.connection.query(
                ...
                function (err, res) {
                    ...
                    if (newEmployeeResults.addEmployeeHasMgr === true) {
                        ...
                        console.log(`Awesome  You have added` `${newEmployeeResults.addEmployeeNameF} ${newEmployeeResults.addEmployeeNameL}`.underline.brightGreen`to the team!`)
                        continueOption();
                    }
                }
            )
